...
vector<std::string> imageFiles = {"textures/background.jpg", "textures/sun.jpg",
                                  "textures/earth.jpg", "textures/moon.jpg"};
...
void objectDraw() { objectDraw(pipeline); }

void objectDraw(VkPipeline pipeline) {
  if (!vklFrameworkInitialized()) {
    VKL_EXIT_WITH_ERROR("Framework not initialized. Ensure to invoke "
                        "vklFrameworkInitialized beforehand!");
  }
  ...
  cb.bindVertexBuffers(0u, {vk::Buffer{objectGetVertexBuffer()}},
                       {vk::DeviceSize{0}});

  // upload the matrix to the GPU via push constants
  objectUpdateConstants();
  vklSetPushConstants(pipeline,
                      VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT,
                      &pushConstants, sizeof(ObjectPushConstants));

  cb.draw(
      6u, 1u, 0u,
      0u); // it's a full-screen quad, there's only six vertices by definition
}
...
void objectCreatePipeline() {
  ...
  config.inputAttributeDescriptions.emplace_back(
...
void objectCreateCamera(GLFWwindow *window) {
  ...
  camera = vklCreateCamera(window, vklCreatePerspectiveProjectionMatrix(
...
TextureInfo objectCreateTextureImage(const char *file) {
  ...
  VkDeviceSize imageSize =
      texWidth * texHeight * STBI_rgb_alpha * sizeof(stbi_uc);

  if (!pixels)
    throw std::runtime_error("failed to load texture image!");

  VkBuffer stagingBuffer = vklCreateHostCoherentBufferAndUploadData(
      pixels, static_cast<size_t>(imageSize), VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
  ...
  createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB,
              ...
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, retVal.image, retVal.memory);

  transitionImageLayout(retVal.image, VK_FORMAT_R8G8B8A8_SRGB,
                        VK_IMAGE_LAYOUT_UNDEFINED,
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  copyBufferToImage(stagingBuffer, retVal.image,
...
void createTextureImageViews() {
  ...
  for (auto &texture : textures) {
    ...
    if (vkCreateImageView(mDevice, &viewInfo, nullptr, &(texture.view)) !=
        VK_SUCCESS) {
...
void createTextureSamplers() {
  ...
  samplerInfo.anisotropyEnable =
      VK_FALSE; // disabled since we have not talked about anisotropic filtering
  samplerInfo.borderColor = VK_BORDER_COLOR_INT_OPAQUE_BLACK;
  samplerInfo.unnormalizedCoordinates =
      VK_FALSE; // Use normalized texture coordinates (in the [0,1] range)
...
void createDescriptorPool() {
  ...
  if (vkCreateDescriptorPool(mDevice, &poolInfo, nullptr, &descriptorPool) !=
      VK_SUCCESS) {
