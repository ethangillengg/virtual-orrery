...
/*!
 *	This callback function gets invoked by GLFW whenever a GLFW error
 *occured.
 */
void errorCallbackFromGlfw(int error, const char *description);

/*!
 *	Function that is invoked by GLFW to handle key events like key presses
 *or key releases. If the ESC key has been pressed, the window will be marked
...
/*!
 *	Function that can be used to query whether or not currently, i.e. NOW, a
 *certain button is pressed down, or not.
 *  @param	glfw_key_code	One of the GLFW key codes.
 *                          I.e., use one of the defines that start with
 *GLFW_KEY_*
 *  @return True if the given key is currently pressed down, false otherwise
 *(i.e. released).
 */
bool isKeyDown(int glfw_key_code);

/*!
 *	Determine the Vulkan instance extensions that are required by GLFW and
 *Vulkan Launchpad. Required extensions from both sources are combined into one
 *single vector (i.e., in contiguous memory, so that they can easily be passed
 *to: VkInstanceCreateInfo::enabledExtensionCount and to
 *VkInstanceCreateInfo::ppEnabledExtensionNames.
 *	@return     A std::vector of const char* elements, containing all
 *required instance extensions.
 *	@example    std::vector<const char*> extensions =
 *getRequiredInstanceExtensions(); VkInstanceCreateInfo create_info    = {};
...
/*!
 *	Based on the given physical device and the surface, select a queue
 *family which supports both, graphics and presentation to the given surface.
 *Return the INDEX of an appropriate queue family!
 *	@return		The index of a queue family which supports the required
...
static std::set<std::string> QuerySupportedLayers() {
  ...
  if (layersAreSet == false) {
    ...
    vkEnumerateInstanceLayerProperties(&count, layers.data()); // populate
                                                               // buffer
...
FilterSupportedLayers(std::vector<char const *> const &layers) {
...
int main(int argc, char **argv) {
  ...
  // Install a callback function, which gets invoked whenever a GLFW error
  // occurred:
  ...
  glfwWindowHint(
      GLFW_CLIENT_API,
      GLFW_NO_API); // No need to create a graphics context for Vulkan
  ...
  if (!window) {
    ...
    VKL_LOG("2) You haven't implemented the first task, which is creating a "
            "window with GLFW.");
  ...
  // Describe some meta data about this application, and define which Vulkan API
  // version is required:
  VkApplicationInfo application_info = {}; // Zero-initialize every member
  application_info.sType =
      VK_STRUCTURE_TYPE_APPLICATION_INFO; // Set this struct instance's type
  ...
  application_info.apiVersion =
      VK_API_VERSION_1_1; // Your system needs to support this Vulkan API
                          // version.

  // We'll require some extensions (e.g., for presenting something on a window
  // surface, and more):
  std::vector<const char *> required_extensions =
      getRequiredInstanceExtensions();

  // Layers enable additional functionality. We'd like to enable the standard
  // validation layer, so that we get meaningful and descriptive error messages
  // whenever we mess up something:
  std::vector<const char *> enabled_layers{};
  if (!hlpIsInstanceLayerSupported("VK_LAYER_KHRONOS_validation")) {
    VKL_WARNING("Validation layers are not supported!");
    // VKL_EXIT_WITH_ERROR("Validation layer \"VK_LAYER_KHRONOS_validation\" is
    // not supported.");
  } else {
  ...
  VkInstanceCreateInfo instance_create_info =
      {}; // Zero-initialize every member
  instance_create_info.sType =
      VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO; // Set this struct instance's type
  instance_create_info.pApplicationInfo = &application_info;
  // Hook in required_extensions using
  // VkInstanceCreateInfo::enabledExtensionCount and
  // VkInstanceCreateInfo::ppEnabledExtensionNames! Hook in enabled_layers using
  // VkInstanceCreateInfo::enabledLayerCount and
  ...
  // Use vkCreateInstance to create a vulkan instance handle! Assign it to
  // vk_instance!
  VkResult result =
  ...
  // Use glfwCreateWindowSurface to create a window surface! Assign its handle
  // to vk_surface!
  ...
  // Use vkEnumeratePhysicalDevices get all the available physical device
  // handles! Select one that is suitable using hlpSelectPhysicalDeviceIndex and
  // assign it to vk_physical_device!
  ...
  vk_physical_device = physical_devices[hlpSelectPhysicalDeviceIndex(
      physical_devices, vk_surface)];
  ...
  // Find a suitable queue family and assign its index to the following
  // variable: Hint: Use selectQueueFamilyIndex, but complete its implementation
  // before!
  // uint32_t selected_queue_family_index =
  // std::numeric_limits<uint32_t>::max();
  uint32_t selected_queue_family_index =
      selectQueueFamilyIndex(vk_physical_device, vk_surface);
  ...
  // - Use VkDeviceCreateInfo::enabledExtensionCount and
  // VkDeviceCreateInfo::ppEnabledExtensionNames
  //   to enable the VK_KHR_SWAPCHAIN_EXTENSION_NAME device extension!
  // - The other parameters are not required (ensure that they are
  // zero-initialized).
  //   Finally, use vkCreateDevice to create the device and assign its handle to
  //   vk_device!
  ...
  auto const supportedExtension = FilterSupportedLayers(
      std::vector<char const *>{VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME});
  device_extensions.insert(device_extensions.end(), supportedExtension.begin(),
                           supportedExtension.end());
  ...
  result = vkCreateDevice(vk_physical_device, &device_create_info, nullptr,
                          &vk_device);
  VKL_CHECK_VULKAN_RESULT(result);
  ...
  // After device creation, use vkGetDeviceQueue to get the one and only created
  // queue! Assign its handle to vk_queue!
  ...
  // Init the framework:
  if (!vklInitFramework(vk_instance, vk_surface, vk_physical_device, vk_device,
                        vk_queue, swapchain_config)) {
...
VklSwapchainConfig GenerateSwapChainConfig() {
  VkSurfaceCapabilitiesKHR surface_capabilities =
      hlpGetPhysicalDeviceSurfaceCapabilities(vk_physical_device, vk_surface);
  ...
  VkSurfaceFormatKHR surface_format =
      hlpGetSurfaceImageFormat(vk_physical_device, vk_surface);
  ...
  // Create the swapchain using vkCreateSwapchainKHR and assign its handle to
  // vk_swapchain!

  auto result = vkCreateSwapchainKHR(vk_device, &swapchain_create_info, nullptr,
                                     &vk_swapchain);
  VKL_CHECK_VULKAN_RESULT(result);
  ...
  // Create a vector of VkImages with enough memory for all the swap chain's
  // images:
  std::vector<VkImage> swap_chain_images(surface_capabilities.minImageCount);
  // Use vkGetSwapchainImagesKHR to write VkImage handles into
  ...
  for (VkImage vk_image : swap_chain_images) {
    ...
    framebufferData.colorAttachmentImageDetails.imageFormat =
        surface_format.format;
    framebufferData.colorAttachmentImageDetails.imageUsage =
...
bool isKeyDown(int glfw_key_code) { return g_isGlfwKeyDown[glfw_key_code]; }
...
std::vector<const char *> getRequiredInstanceExtensions() {
  ...
  const char **vkl_extensions =
      vklGetRequiredInstanceExtensions(&num_vkl_extensions);

  // Merge both arrays of extensions:
  std::vector<const char *> all_required_extensions(
      glfw_extensions, glfw_extensions + num_glfw_extensions);
...
uint32_t selectQueueFamilyIndex(VkPhysicalDevice physical_device,
  ...
  VKL_EXIT_WITH_ERROR("Unable to find a suitable queue family that supports "
                      "graphics and presentation on the same queue.");
}
